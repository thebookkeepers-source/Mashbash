...
void main() {
  test('Mashbash launcher and in-app branding assets are available', () {
    final launcherConfig = File('flutter_launcher_icons.yaml').readAsStringSync();
    ...
    expect(launcherConfig, contains('adaptive_icon_background: "#8B0000"'));
    expect(launcherConfig, contains('adaptive_icon_foreground: "assets/branding/app_icon_foreground.png"'));
  });
  ...
  test('Google login remains hidden until OAuth setup is enabled', () {
    ...
    expect(authScreen, contains('Email or mobile number'));
    expect(authScreen, isNot(contains('Customer email / staff mobile')));
    expect(authScreen, isNot(contains('Customers use email')));
    expect(authScreen, isNot(contains('Staff accounts are created securely')));
  });
