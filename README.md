...
The `Android Release` GitHub Actions workflow runs manually from the current selected branch or commit. Add these repository Actions secrets before running it:
...
Create `ANDROID_KEYSTORE_BASE64` from the binary keystore without committing it. The workflow validates and decodes the key only on the runner, verifies the production source and exact release commit, builds signed `app-release.apk`, uploads it in the `app-release-apk` artifact, then removes the temporary signing files.
...
GitHub Actions runs analyze, tests, debug APK build, and release APK/AAB compile checks. It uploads `mashbash-debug-apk` as a seven-day workflow artifact. The `Android Release` workflow uploads `app-release-apk` only when all signing and Supabase secrets are configured.
